...
实验设置上，本研究在两个benchmark上评估了GoS的表现。

SkillsBench包含跨越11个领域的多样化真实世界技术任务,并配有精心策划的技能:结构化的程序性知识包(指令、代码模板、资源),用于在推理时增强 LLM 代理。任务领域涵盖复杂的技术工作,如宏观经济去趋势、电网可行性分析、3D 扫描分析、金融建模和地震相位拾取 。

ALFWorld是一个交互式模拟器,它将文本描述和命令与物理具身机器人环境对齐,由 TextWorld(一个交互式文本游戏引擎)和 ALFRED 数据集结合而成。其任务涉及多步骤的家庭活动,例如导航房间、查找物体并操作它们。

研究在完整的140个回合上进行评估

baseline方法包括**Vanilla Skills**和**Vector Skills**。Vector Skills 使用与 GoS 相同的 嵌入模型,即 openai/text-embedding-3-large(3072 维),通过语义相似性检索一个受限的技能集。它将图结构的影响与检索时压缩的普遍好处分离开来。GoS 使用 与 Vector Skills 相同的基础嵌入模型,但将平面最近邻检索替换为在技能图上的结构感知检索。
